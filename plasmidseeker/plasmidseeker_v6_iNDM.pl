## This v6 script is latest plasmidseeker script. To run just type: time perl PlasmidAssignment_Pipeline_v5.pl 
...
$cmd="time perl /storage/apps/PlasmidSeeker/plasmidseeker.pl -d /storage/apps/PlasmidSeeker/db4_cdc_NDM_only -i $tmp.unzipped.fastq -b $fasta -o $tmp\_plasmidseeker_results.txt -t 48 -f 80"; 
...
#	  	        push (@array, $a);
			#print "$a\n";
			($plasmid_file_number) = $a =~ m/\/storage\/apps\/PlasmidSeeker\/db4_cdc_NDM_only\/(plasmid_.*.fna)_20.list/g;
			#print "this is plasmid number $plasmid_file_number\n";
			#$q_plasmid_file_number=quotemeta($plasmid_file_number);
			$grepp=qx(grep "$plasmid_file_number" /storage/apps/PlasmidSeeker/db4_cdc_NDM_only/names.txt);
			#system($grepp);
			#print $grepp;
			if ($grepp =~ m/\|(.*)\|/)
			{
			...
			for($k=$j;$k<=$#plasmidseeker_file;$k++)
				{
					...
					($plasmid_file_number_hp) = $hpvalue =~ m/\/storage\/apps\/PlasmidSeeker\/db4_cdc_NDM_only\/(plasmid_.*.fna)_20.list/g;
					#	print "plasmidnumber is this: $plasmid_file_number_hp\t";
					$grepp_hp=qx(grep "$plasmid_file_number_hp" /storage/apps/PlasmidSeeker/db4_cdc_NDM_only/names.txt);
...
chdir "/storage/data/DATA4/analysis/Datta/7iNDM_PlasmidSeeker/Species/EC";
