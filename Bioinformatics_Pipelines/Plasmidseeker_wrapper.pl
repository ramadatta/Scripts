...
$Final_Directory="/storage/data/DATA4/analysis/5Glorijoy/2019/Plasmidseeker/Split_Samples_CPGenes/NDM";
...
$Plasmidseeker_DB="/storage/apps/PlasmidSeeker/db4_cdc_NDM_only_v2_CPE_Transmission";
...
foreach $a (@plasmidseeker_file)
{
		...
		if($a=~/.*\/.*/)
		{
		($plasmid_file_number) = $a =~ m/\/storage\/apps\/PlasmidSeeker\/db4_cdc_NDM_only_v2_CPE_Transmission\/(plasmid_.*.fna)_20.list/g;
			print "this is plasmid number ---$plasmid_file_number\n";

			$grepp=qx(grep "$plasmid_file_number" /storage/apps/PlasmidSeeker/db4_cdc_NDM_only_v2_CPE_Transmission/names.txt);
			if ($grepp =~ m/\|(.*)\|/)
			{
				$accn=$1;
				#print "$accn\n";
...
#`top1=$(awk '{print $NF}' Plasmid_list_for_QueryContigs_withAlignment_prcnt.txt | head -1)`;
#system( q@ awk -v d1="$top1" '$NF == d1' Plasmid_list_for_QueryContigs_withAlignment_prcnt.txt >Plasmid_list_for_QueryContigs_withAlignment_prcnt_All_Top_Best.txt @);
...
=cut
	if( grep -f, glob '*_cutadapt.fastq' ) ##cases without gz files, everything from above is same except that here files are already unzipped
	{
...
foreach $b (@plasmidseeker_file)
{
		...
		if($b=~/.*\/.*/)
		{
		($plasmid_file_number) = $b =~ m/\/storage\/apps\/PlasmidSeeker\/db4_cdc_NDM_only_v2_CPE_Transmission\/(plasmid_.*.fna)_20.list/g;
			print "this is plasmid number ---$plasmid_file_number\n";
...
#system( q@for d in $(ls *_cpContigs_vs_PlasmidseekerPlasmids_blastresults.txt | grep -v 'test'); do echo -n "$d       "; sampName=`echo "$d" |sed 's/_cpContigs_vs_PlasmidseekerPlasmids_blastresults.txt//g'`;  accn=`head $d | cut -f2 | head -1 | grep -Po ".*\.._" | sed 's/_$//g' | sed 's/gbk://g'`; echo -n "$accn      "; if [ -n "$accn" ]; then plasmid_num=`grep "$accn" /storage/apps/PlasmidSeeker/db4_cdc_NDM_only_v2_CPE_Transmission/names.txt | cut -f1`; echo -n "$plasmid_num    "; partd=`echo $d | cut -f1,2 -d "/"`; grep "$plasmid_num" *_plasmidseeker_results.txt; fi; echo -n " "; cat *_cpContigs_vs_PlasmidseekerPlasmids_blastresults_no_qcov_cutoff.txt; done | paste  - - | sed 's/ \+ /\t/g' | sed 's/ /_/g' | awk '{print $12,$2,$4,$5,$6,$9,$13,$14,$15,$16}' >Final_blast_kmer_cov.txt@);
